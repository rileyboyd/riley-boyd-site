  ...
  mc.on('panleft panright panup pandown', (e) => {
    ...
    if (isFirst) {
      ...
      if ($rightSide && $rightSide.length) {
        ...
        // open
        if (
          wndW - startSwipe <= swipeStartSize &&
          !$rightSide.hasClass('open') &&
          !$leftSide.hasClass('open')
        ) {
          openNav = 1
          isRightSide = 1
      ...
      if (
        isRightSide &&
        ((openNav && swipeDir === 'left') || (closeNav && swipeDir === 'right'))
      ) {
        // open side navbar
        if (openNav) {
        ...
        isLeftSide &&
        ((openNav && swipeDir === 'right') || (closeNav && swipeDir === 'left'))
      ) {
        // open mobile navbar
        if (openNav) {

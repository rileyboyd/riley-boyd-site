...
function initFullPage() {
  ...
  function slideShow(index, effect = 'fade', force) {
    if (
      (typeof slides[index] !== 'undefined' && curIndex !== index && !isBusy) ||
      force
    ) {
      isBusy = 1
  ...
  $wnd.on('DOMMouseScroll mousewheel wheel', (e) => {
    ...
    if (
      Math.abs(e.originalEvent.deltaY) < 2 ||
      !$(e.target).parents('.rb-fullpage-portfolio').length
    ) {
      return
    }

...
function initPluginJarallax() {
  ...
  const $parallaxHeader = $(
    '.rb-header-title-parallax, .rb-header-title-parallax-opacity'
  ).eq(0)
  if ($parallaxHeader.length) {
    const $headerImage = $parallaxHeader.find('> .bg-image > div:eq(0)')
    const $headerContent = $parallaxHeader.find('> .bg-image ~ *')
    const headerParallaxScroll = $parallaxHeader.hasClass(
      'rb-header-title-parallax'
    )
    const headerParallaxOpacity = $parallaxHeader.hasClass(
      'rb-header-title-parallax-opacity'
    )
    $parallaxHeader.jarallax({
      ...
      onScroll(calc) {
        ...
        if (headerParallaxOpacity) {
          $headerContent.css({
            opacity:
              calc.visiblePercent < 0 ||
              (calc.beforeTop > 0 ? 1 : calc.visiblePercent),
          })
        }
      },
    })
  ...
  const $parallaxFooter = $(
    '.rb-footer-parallax, .rb-footer-parallax-opacity'
  ).eq(0)
  if ($parallaxFooter.length) {
    ...
    const footerParallaxOpacity = $parallaxFooter.hasClass(
      'rb-footer-parallax-opacity'
    )

...
function initNavbar() {
  ...
  function correctDropdown($item) {
    if ($item.parent().is('.rb-nav')) {
      ...
      if (currentLeftPost > itemRect.left) {
        css.marginLeft =
          (css.marginLeft || 0) - (currentLeftPost - itemRect.left)
      }
      ...
      css.marginTop =
        $parent.innerHeight() - $dropdown.offset().top + $parent.offset().top
  ...
  let dropdownTimeout
  $navbarTop
    .on('mouseenter', 'li.rb-drop-item', function () {
      const $item = $(this)
      const $openedSiblings = $item
        .siblings('.open')
  ...
  self.throttleScroll((type, scroll) => {
    ...
    // add solid color
    if (
      $navbarTop.hasClass('rb-navbar-transparent') &&
      $navbarTop.hasClass('rb-navbar-sticky')
    ) {
